...
def prompt_model(dataset, model_name = "deepseek-ai/deepseek-coder-6.7b-base", quantization = True):
    ...
    for case in dataset:
        ...
        # 2. 找到代码块的基准缩进
        #    将生成内容按行分割，并找到第一个非空代码行。
        lines = completion.split('\n')
        code_lines = [line for line in lines if line.strip()]

        if not code_lines:
            # 如果模型没有生成任何有效代码，则处理结果为空字符串。
            response_processed = ""
        else:
            # 3. 计算基准缩进的长度
            #    以第一个有效代码行为准，计算它开头的空格数。这个长度就是整个代码块的“基准缩进”。
            #    比如，如果第一行是 "  if n > 0:"，那么基准缩进就是2个空格。
            ...
            for line in lines:
                ...
                # 这是关键的停止条件：
                # 如果当前行的缩进比基准缩进还要小，说明代码块已经结束了。
                # (例如，一个新函数的`def`语句，或者全局变量，它们的缩进为0，小于函数体的缩进)
                # 此时，我们立即停止处理，从而巧妙地忽略了所有后续的无关代码。
                if current_indent_len < base_indent_len:
                    break
                
                # 修正缩进：
                # a. 移除原始的基准缩进 (line[base_indent_len:])
                # b. 添加标准的4个空格缩进 ("    " + ...)
                # 这样做既能统一整个代码块的缩进，又能保持其内部的相对缩进结构（比如循环体内的代码）。
                standard_indent = "    "
                reindented_line = standard_indent + line[base_indent_len:]
                processed_block.append(reindented_line)
                    
                # 5. 生成最终结果
                #    将处理好的所有行重新组合成一个字符串，并去掉末尾多余的空白。
